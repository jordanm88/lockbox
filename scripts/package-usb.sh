...
if [[ "$OS_NAME" == "Darwin" ]]; then
  ...
  if [[ ! -f "$RCLONE_DEST" ]]; then
    echo "NOTE: no rclone binary at Tools/mac/rclone yet — download from https://rclone.org/downloads/, copy it in, then chmod +x it."
  fi
  ...
  if [[ ! -f "$RCLONE_DEST" ]]; then
    echo "NOTE: no rclone binary at Tools/linux/rclone yet — download from https://rclone.org/downloads/, copy it in, then chmod +x it."
  fi
