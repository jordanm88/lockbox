/**
 * Playwright-based resolver to follow homepage download flows and obtain final archive URLs + SHA256.
 *
 * Usage:
 * 1) `npm i -D playwright` (or `npm i -D playwright@latest`)
 * 2) `node scripts/playwright/resolve_catalog.js`
 *
 * Note: Running this requires network access and Playwright browser binaries.
 */
const fs = require('fs');
...
(async () => {
  ...
  for (const app of catalog.apps) {
    ...
    try {
      ...
      // heuristic: find anchor or button with text 'Download' or 'download'
      ...
      // fallback: look for links that look like archives
      if (!finalUrl) {
      ...
      // naive: write sha to first target (windows) if present
