    ...
    <div className="space-y-4">
      <div className="neo-panel overflow-hidden">
        <div className="flex flex-col gap-4 border-b border-slate-200/80 bg-white px-4 py-4 sm:px-5 sm:py-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <div className="neo-border flex overflow-hidden bg-white">
                <button
                  type="button"
                  onClick={() => setViewMode("list")}
                  className={`px-3 py-2 text-sm font-semibold transition ${viewMode === "list" ? "bg-neo-blue text-white" : "text-slate-600 hover:bg-slate-50"}`}
                >
                  List
                </button>
                <button
                  ...
                  className={`px-3 py-2 text-sm font-semibold transition ${viewMode === "grid" ? "bg-neo-blue text-white" : "text-slate-600 hover:bg-slate-50"}`}
                >
                  Grid
                </button>
              </div>
        ...
        {viewMode === "list" ? (
          ...
          <div className="p-5">
            {rows.length === 0 ? (
              <div className="py-14 text-center text-sm font-medium text-slate-600">This folder is empty.</div>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {rows.map((row) => {
                  ...
                  return (
                    <div key={row.fullPath} className={`neo-border rounded-2xl p-4 transition hover:-translate-y-0.5 hover:shadow-md ${row.isDir ? "bg-sky-50/50" : "bg-white"}`}>
                      <button
                        type="button"
                        onClick={row.isDir ? () => setCurrentPath(row.fullPath) : () => onView(full)}
                        className="flex w-full items-start gap-3 text-left"
                      >
                        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ring-1 ${row.isDir ? "bg-sky-100 text-sky-700 ring-sky-200" : "bg-slate-100 text-slate-700 ring-slate-200"}`}>
                          {row.isDir ? "📁" : "📄"}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-semibold text-ink">{row.name}</p>
                          <p className="mt-1 text-sm text-slate-500">{row.isDir ? "Folder" : `${Math.max(1, Math.ceil(row.size / 1024))} KB`}</p>
                        </div>
                      </button>
